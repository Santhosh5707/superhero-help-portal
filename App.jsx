...
function HeroCard({hero,onSelect}){
 ...
 return <button className="hero-card" onClick={()=>onSelect(hero)}>
  <div className={`hero-avatar ${hero.color}`}>{hero.id==='dasham'?<img src={dashamHero} alt="Dasham, the Guardian of Kerala"/>:hero.emoji}</div>
...
function App(){
 ...
 const submitRequest=e=>{
  ...
  save([item,...requests]);setModal(false);showToast(`Request ${id} dispatched to ${assigned}`)
 };
 ...
 return <div className="app-shell">
  ...
  {selectedHero&&<div className="modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setSelectedHero(null)}><div className="modal hero-detail"><button className="close" onClick={()=>setSelectedHero(null)}><X/></button><div className={`hero-avatar ${selectedHero.color} large`}>{selectedHero.id==='dasham'?<img src={dashamHero} alt="Dasham, the Guardian of Kerala"/>:selectedHero.emoji}</div><div className="eyebrow">HERO PROFILE</div><h2>{selectedHero.name}</h2><p className="modal-sub">{selectedHero.alias} · {selectedHero.specialty}</p><div className="detail-grid"><div><b>{selectedHero.response}</b><span>Typical response</span></div><div><b>{selectedHero.missions}</b><span>Missions completed</span></div></div><div className="power-row detail-powers">{selectedHero.powers.map(p=><span key={p}>{p}</span>)}</div><button className="outline full" onClick={()=>setSelectedHero(null)}>Back to Hero Directory <ArrowRight size={16}/></button></div></div>}
